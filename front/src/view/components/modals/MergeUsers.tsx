...
export function MergeUsers({ setClose, open }: ModalComponentProps) {
	...
	const onSelectedUserChange = useCallback((_, names: string[]) => {
	...
	return (
		<Dialog open={open} onClose={setClose} TransitionComponent={Transition}>
			...
			<DialogContent dividers>
				<Stack p={2} spacing={3} minWidth={400}>
					<Autocomplete
						...
						freeSolo
						renderInput={params => (
							<TextField helperText={"Le nom de l'utilisateur après le merge"}
									   {...params}
									   label={"Prochain nom"}
							/>
						)}
					...
					<Autocomplete
						...
						options={users}
						renderInput={params => (
							<TextField helperText={"Users to merge"}
									   {...params}
									   label={"Utilisateur à unir"}
							/>
						)}
			...
			<DialogActions>
				<Box p={1}>
					<Button variant={"outlined"} color={"success"} onClick={merge}>Merge</Button>
				</Box>
			</DialogActions>
		</Dialog>
	);
