...
import { createOrderRecord, deleteOrderRecord, setAlteringOrder, setAlteringRecord } from "../../../store/module/orders/orders.action";
...
export function EditOrder() {
	...
	return (
		<Dialog open={Boolean(order)} onClose={deleteOrderFn}>
						...
						<Box>
							<Stack direction={"row"} spacing={3}>
								...
								<IconButton size={"small"} sx={{ p: 0 }} onClick={addRecord}>
									<AddCircleOutlineIcon sx={{ width: 30, height: 30 }} color={"secondary"} />
								</IconButton>
							</Stack>
			...
			<DialogActions>
				<Button color={"inherit"} onClick={deleteOrderFn}>
					Fermer
				</Button>
